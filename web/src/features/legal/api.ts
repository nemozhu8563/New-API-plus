...
export async function getUserAgreement(locale?: string): Promise<LegalDocumentResponse> {
  const res = await api.get<LegalDocumentResponse>('/api/user-agreement', {
    headers: { 'Cache-Control': null }, params: locale ? { locale } : undefined,
  })
  return res.data
}

export async function getPrivacyPolicy(locale?: string): Promise<LegalDocumentResponse> {
