...
func buildTaskQuery(userId *int, queryParams SyncTaskQueryParams) *gorm.DB {
	query := DB.Model(&Task{})
	if userId != nil {
...
func CountAllTaskForExport(queryParams SyncTaskQueryParams) (int64, error) {
	var total int64
	err := buildTaskQuery(nil, queryParams).Count(&total).Error
	return total, err
}

func CountUserTaskForExport(userId int, queryParams SyncTaskQueryParams) (int64, error) {
	var total int64
	err := buildTaskQuery(&userId, queryParams).Count(&total).Error
	return total, err
}

func streamTaskForExport(ctx context.Context, query *gorm.DB, yield func(*Task) error) error {
...
func StreamAllTaskForExport(ctx context.Context, queryParams SyncTaskQueryParams, yield func(*Task) error) error {
	query := buildTaskQuery(nil, queryParams)
	var userIds []int
	...
	for start := 0; start < len(userIds); start += usageLogExportLookupBatchSize {
		end := min(start+usageLogExportLookupBatchSize, len(userIds))
		var users []struct {
			Id       int
			Username string
		}
	...
	return streamTaskForExport(ctx, query, func(task *Task) error {
		task.Username = usernames[task.UserId]
		return yield(task)
	})
}

func StreamUserTaskForExport(ctx context.Context, userId int, queryParams SyncTaskQueryParams, yield func(*Task) error) error {
	return streamTaskForExport(ctx, buildTaskQuery(&userId, queryParams), yield)
}
