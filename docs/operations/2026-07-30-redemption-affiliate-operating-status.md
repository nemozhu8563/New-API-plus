...
>
> 证明边界：功能已在 GreenCloud 独立测试实例完成 PostgreSQL 迁移和匿名访问巡检；测试实例已于 2026-07-31 切换到提交 `c804b2a2` 的不可变镜像并通过健康检查。代码尚未推送远端，也未使用目标账号完成业务 E2E，因此不能视为正式上线。
...
- 当前提交尚未推送远端；已发布到 GreenCloud 测试实例，尚未正式生产发布。
- 后端账务与接口测试使用本地 SQLite；GreenCloud 测试库已完成 PostgreSQL 迁移，但尚未对 MySQL 执行实际迁移，也未完成 PostgreSQL/MySQL 并发验证。
- GreenCloud 测试实例当前使用 `new-api:test-20260731-c804b2a`（commit `c804b2a2`，digest `sha256:77c80e9e…57134e2`），已不再使用提交前的 dirty 镜像快照。
