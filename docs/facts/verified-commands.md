...
| 重建 GreenCloud 测试应用 | GreenCloud `/srv/new-api-test/compose.yaml`、`new-api-test` | `docker compose -f /srv/new-api-test/compose.yaml up -d --no-deps --no-build --pull never --force-recreate new-api-test` | `docs/operations/greencloud-service-migration-sop.md`、2026-09-07 切换记录 | 2026-09-07（Asia/Shanghai） | 已确认：执行成功；仅重建测试应用为 `new-api:new-api-test-20260907T135216Z-22a1b1d82`，最终 `running/healthy`、重启次数 `0`；测试本机、`test.tryvalo.com`、`/wallet`、`/login` 与公开套餐 API 回读均正常 |
| 重建 GreenCloud 正式应用 | GreenCloud `/srv/new-api/compose.yaml`、`new-api` | `docker compose --env-file /srv/new-api/env/images.env -f /srv/new-api/compose.yaml up -d --no-deps --no-build --pull never --force-recreate new-api` | `docs/operations/greencloud-service-migration-sop.md`、2026-09-07 切换记录 | 2026-09-07（Asia/Shanghai） | 已确认：执行成功；仅重建正式应用为 `new-api:new-api-release-20260907T135216Z-22a1b1d82`，最终 `running/healthy`、重启次数 `0`，PostgreSQL/Redis 未重建；本机、`tryvalo.com`、`api.tryvalo.com`、`new.tryvalo.com` 的 `/api/status` 与正式 `/wallet`、`/login` 均为 HTTP `200` |
