...
| 重建 GreenCloud 正式应用 | GreenCloud `/srv/new-api/compose.yaml`、`new-api` | `docker compose --env-file /srv/new-api/env/images.env -f /srv/new-api/compose.yaml up -d --no-deps --no-build --pull never --force-recreate new-api` | `docs/operations/greencloud-service-migration-sop.md`、正式发布记录 | 2026-09-03（Asia/Shanghai） | 已确认：执行成功；仅重建正式应用为 `new-api:new-api-release-20260903T094447Z-e40d88d1535`，最终 `running/healthy`、重启次数 `0`，PostgreSQL/Redis 未重建；本机及 `api.tryvalo.com`、`new.tryvalo.com` 的 `/api/status` 均为 HTTP `200` |
