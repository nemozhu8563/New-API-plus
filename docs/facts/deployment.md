...
| 生产 | 已确认（2026-09-08 11:10:45 发布，Asia/Shanghai）：GreenCloud 正式应用运行提交 `3abc3c557` 的 `linux/amd64` 镜像 `new-api:new-api-release-20260908-wallet-3abc3c557`，`running/healthy`、重启 `0`。充值累计钱包容量与单次计费边界分离；一笔原失败 Live 充值重发后成功入账。环境变量和 Compose 未变，PostgreSQL、Redis 未重建；本机及两个正式状态入口 HTTP `200` | `api.tryvalo.com`、`new.tryvalo.com`、`tryvalo.com` | GreenCloud Docker/数据库、HTTP、Stripe Workbench；[发布记录](../operations/2026-09-08-stripe-wallet-capacity-release.md) | 2026-09-08 |

## 当前远端快照

- 最新发布为 2026-09-08 11:10:45（Asia/Shanghai）的正式钱包容量修复，镜像 ID `sha256:3b53d4fccc449e54d927474069e42e84e3fbc3c60c0629618b4c2de2a8f17358`。测试未更新。备份、配置摘要、健康、Live 入账和回滚边界见 [钱包容量发布记录](../operations/2026-09-08-stripe-wallet-capacity-release.md)；下列较早记录不表示测试与正式目前仍运行同一镜像。
