...
| 生产 | 已确认（2026-09-08 09:38:42，Asia/Shanghai）：GreenCloud 正式应用运行提交 `369141e27` 的不可变 `linux/amd64` 镜像 `new-api:new-api-release-20260908T012416Z-369141e27`，容器 `running/healthy`、重启次数 `0`；运行时已配置非空的 Live 订阅 Checkout PMC，实际标识不记录在仓库或 Facts。PostgreSQL、Redis 未重建。正式本机、`api.tryvalo.com` 和 `new.tryvalo.com` 状态接口均为 HTTP `200`；本机公开套餐接口返回 Live Plan `1/2/3`，均标记 Stripe Checkout 可用。真实 Live Checkout 的微信展示和付款闭环仍待定 | `api.tryvalo.com`、`new.tryvalo.com`、`tryvalo.com` | GreenCloud Docker/Compose、运行时环境变量存在性检查、本机和公网 HTTP、公开套餐 API、备份校验 | 2026-09-08 |
