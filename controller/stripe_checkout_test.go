...
func TestGenStripeLinkUsesContextIdempotencyAndMetadata(t *testing.T) {
	...
	require.NotNil(t, captured.ManagedPayments)
	require.NotNil(t, captured.ManagedPayments.Enabled)
	assert.False(t, *captured.ManagedPayments.Enabled)
	assert.Empty(t, captured.PaymentMethodTypes)
