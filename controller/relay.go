...
) (*model.Channel, hosttypes.PriceData, *types.NewAPIError) {
	channel, channelErr := getChannel(c, info, retryParam)
	if channelErr != nil {
		logger.LogError(c, channelErr.Error())
		return nil, hosttypes.PriceData{}, channelErr
	}

	priceData, err := helper.ModelPriceHelper(c, info, promptTokens, meta)
	if err != nil {
		return nil, hosttypes.PriceData{}, types.NewError(err, types.ErrorCodeModelPriceError, types.ErrOptionWithStatusCode(http.StatusBadRequest))
	}
...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for retryParam.GetRetry() <= common.RetryTimes || emergencyAttemptsRemaining > 0 {
		...
		case types.RelayFormatClaude:
			relayInfo.ResetAttemptConversionMeta(tokens)
